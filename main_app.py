...
class ProcessingThread(QObject):
    ...
    Toteutus käyttää Pythonin threading.Thread(daemon=True) QThreadin
    sijaan. Daemon-säie sammuu automaattisesti kun pääprosessi suljetaan,
    joten ohjelma ei jää roikkumaan kun käyttäjä sulkee ikkunan.

    QThread.quit() toimii vain jos säikeessä pyörii Qt:n oma event loop
    (exec_()), mitä tässä ei ole – siksi vanha QThread-toteutus ei sammunut
    oikein.
    ...
    def __init__(self, func, *args, **kwargs):
        ...
        self.args = args
        self.kwargs = kwargs
        # daemon=True: säie sammuu automaattisesti pääprosessin mukana
        self._thread = threading.Thread(target=self._run, daemon=True)

    def start(self):
        """Käynnistää taustasäikeen."""
        self._thread.start()

    def isRunning(self) -> bool:
        """Palauttaa True jos säie on vielä käynnissä."""
        return self._thread.is_alive()

    def wait(self, msecs: int = -1):
        """Odottaa säikeen päättymistä (enintään msecs millisekuntia)."""
        timeout = msecs / 1000.0 if msecs >= 0 else None
        self._thread.join(timeout=timeout)

    def _run(self):
        """Sisäinen suoritusmetodi – kutsutaan daemon-säikeessä."""
        try:
...
class LaserPlotterApp(QMainWindow):
    ...
    Layout:
      ...
      Oikea paneeli (välilehdet):
        ...
        - Alue & Laite: piirtoalueen mitat ja tallennushakemisto (Output/)
    """
    ...
    def __init__(self):
        ...
        # Tallennushakemisto "Output" luodaan ohjelman omaan hakemistoon.
        # Path(__file__).resolve().parent antaa kansion jossa main_app.py sijaitsee,
        # riippumatta siitä mistä hakemistosta ohjelma käynnistetään.
    ...
    def _build_area_tab(self) -> QWidget:
        ...
        dgl = QVBoxLayout(dir_grp)
        # Polku on kiinteä (ohjelman hakemisto / Output) – ei muutettavissa
        self.lbl_outdir = QLabel(str(self.output_dir))
        self.lbl_outdir.setWordWrap(True)
        self.lbl_outdir.setStyleSheet(
            "color: #7ec8e3; font-family: monospace; font-size: 11px; "
            "background: #0a0a1e; padding: 4px 6px; border-radius: 3px;"
        )
        dgl.addWidget(self.lbl_outdir)
        lbl_info = QLabel("Kaikki tiedostot tallennetaan automaattisesti tähän kansioon.")
        lbl_info.setWordWrap(True)
        lbl_info.setStyleSheet("color: #888; font-size: 11px;")
        dgl.addWidget(lbl_info)
    ...
    def generate_gcode(self):
        ...
        # Automaattitallennus Output-kansioon heti generoinnin jälkeen,
        # samoin kuin tuloskuvatkin tallennetaan automaattisesti.
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        method_label = (
            self.method_combo.currentText().split("–")[0].strip()
        )
        auto_path = self.output_dir / f"gcode_{method_label}_{device}_{ts}.gcode"
        auto_path.write_text(gcode, encoding="utf-8")

        self.statusBar().showMessage(
            f"G-koodi generoitu – {line_count} riviä  →  {auto_path.name}"
        )
    ...
    def closeEvent(self, event):
        ...
        Kutsuu QApplication.quit() eksplisiittisesti jotta event loop
        varmasti pysähtyy. setQuitOnLastWindowClosed ei ole luotettava
        kaikilla alustoilla (erityisesti Windows).
        """
        event.accept()
        QApplication.quit()
...
def main():
    ...
    window.show()
    app.exec_()
    # Pakota prosessi sammumaan. Tämä ohittaa kaiken mahdollisen
    # Qt:n tai Pythonin cleanup-koodin joka voisi jäädä roikkumaan.
    os._exit(0)
